...
public class PDFDecoder {
  ...
  public final static Set<String> DCT_FILTERS =
    new HashSet<>( Arrays.asList( "DCT", "DCTDecode" ) );
  ...
  public static boolean isLastFilter( PDFObject dict, Set<String> filters )
    throws IOException {
  ...
  public static ByteBuffer decodeStream( PDFObject dict, ByteBuffer streamBuf,
    ...
    if( filter == null ) {
      ...
      boolean specificCryptFilter =
        spec.ary.length != 0 && spec.ary[ 0 ].getStringValue()
                                             .equals( "Crypt" );
      ...
      for( int i = 0; i < spec.ary.length; i++ ) {
        ...
        try {
          ...
          if( enctype == null ) {
            ...
            streamBuf = CCITTFaxDecode.decode( dict,
                                               streamBuf,
                                               spec.params[ i ] );
          }
            ...
            streamBuf = dict.getDecrypter()
                            .decryptBuffer( cfName, null, streamBuf );
          }
          else if( enctype.equals( "JBIG2Decode" ) ) {
            streamBuf = JBig2Decode.decode( dict, streamBuf, spec.params[ i ] );
          }
          else {
            throw new PDFParseException( "Unknown coding method:" + spec.ary[ i ]
              .getStringValue() );
          }
        } catch( Exception e ) {
          throw new PDFParseException( "Problem decoding " + enctype + " " +
                                         "encoded stream!",
                                       e );
        }
   ...
   */
  private static String getCryptFilterName( PDFObject param )
    throws IOException {
    String cfName = PDFDecrypterFactory.CF_IDENTITY;
    if( param != null ) {
