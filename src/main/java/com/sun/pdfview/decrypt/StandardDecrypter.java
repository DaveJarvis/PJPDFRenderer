...
public class StandardDecrypter implements PDFDecrypter {
   ...
   * @param documentId          the contents of the ID entry of the
   *                            document's trailer
   *                            dictionary; can be null, but according to
   *                            the spec, shouldn't be. Is
   *                            expected to be an array of two byte sequences.
   * @param keyBitLength        the length of the key in bits; should be a
   *                            multiple
   *                            of 8 between 40 and 128
   * @param revision            the revision of the Standard encryption
   *                            security handler
   *                            being employed. Should be 2, 3 or 4.
   * @param oValue              the value of the O entry from the Encrypt
   *                            dictionary
   * @param uValue              the value of the U entry from the Encrypt
   *                            dictionary
   * @param pValue              the value of the P entry from the Encrypt
   *                            dictionary
   * @param encryptMetadata     whether metadata is being encrypted, as
   *                            identified
   *                            by the Encrypt dict (with default true if
   ...
   * @throws PDFParseException                        if there's an internal
   * error while testing
   *                                                  cipher availability
   */
   ...
   *
   * @param encrypted          the encrypted content; required by AES
   *                           encryption so
   *                           that the initialisation vector can be
   *                           established
   * @param decryptionKeyBytes the bytes for the decryption key
   * @return a content decryption cypher, ready to accept input
   * @throws PDFParseException                  if the encrypted buffer is
   * malformed or on an
   *                                            internal error
   * @throws NoSuchAlgorithmException           if the cipher algorithm is
   * not supported
   *                                            by the platform
   * @throws NoSuchPaddingException             if the cipher padding is not
   * supported by
   *                                            the platform
  ...
  private int getSaltedContentKeyByteLength( int generalKeyByteLength ) {
    return min( generalKeyByteLength + 5, 16 );
  }
   ...
   * @throws GeneralSecurityException                if there's an error
   * getting required
   *                                                 ciphers, etc.
   *                                                 (unexpected, since a
   *                                                 check for algorithm
  ...
  private byte[] getInitialOwnerPasswordKeyBytes(
    ...
    // Step 2: Initialize the MD5 hash function and pass the result of
    // step 1 as
   ...
   * @param pValue          the P value from the Encrypt dictionary
   * @param encryptMetadata the EncryptMetadata entry from the Encrypt
   *                        dictionary (or false if not present or revision
   *                        &lt;= 3)
  ...
  private byte[] padPassword( byte[] password ) {
    ...
    // â n bytes of the padding string to the end of the password
    // string. If
    ...
    final int numContributingPasswordBytes =
      min( password.length, padded.length );
