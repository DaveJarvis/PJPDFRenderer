...
public class FunctionType4 extends PDFFunction {
  ...
  protected void parse( PDFObject obj ) throws IOException {
    ...
    buf.get( byteA );
    final var scriptContent = new String( byteA, UTF_8 );
  ...
  protected void doFunction( float[] inputs, int inputOffset, float[] outputs,
    ...
    prepareInitialStack( inputs, inputOffset );
		for( final var token : this.tokens ) {
			final var op = OperationSet.getInstance().getOperation( token );
			op.eval( this.stack );
		}
  ...
  private void prepareResult( float[] outputs, int outputOffset ) {
    for( int i = outputOffset; i < outputs.length; i++ ) {
      outputs[ outputs.length - i - 1 ] =
				((Double) this.stack.pop()).floatValue();
    }
  }

  /**
   * Put all input values on the initial stack.
   * All values are pushed as Double because we calculate internally with
   * double.
   */
