...
public class PDFImage {
   ...
   *
   * @throws IOException if {@link PDFDecoder} throws one while evaluating if
   *                     the image is a Jpeg
   */
   ...
   *
   * @param obj        the PDFObject containing the image's dictionary and
   *                   stream
   * @param resources  the current resources
   * @param useAsSMask - flag for switching colors in case image is used as
   *                   sMask
   *                   internally this is needed for handling transparency in
   *                   smask
   *                   images.
   */
   ...
   * @param jpegData a byte buffer if data still requiring the DCDTecode
   *                 filter is
   *                 being used
   */
  protected BufferedImage parseData( byte[] data, ByteBuffer jpegData )
    throws IOException {
    ...
    if( jpegData != null ) {
      ...
      // (http://java.sun.com/javase/6/docs/api/javax/imageio/metadata/doc
      ...
      if( bi == null ) {
        ...
        throw new IOException( decodeEx.getMessage() + ". Maybe installing " +
                                 "JAI for expanded image format "
      ...
      SampleModel sm = cm.createCompatibleSampleModel( getWidth(),
                                                       getHeight() );
      ...
      try {
        ...
        int tempExpectedSize =
          getWidth() * getHeight() * getColorSpace().getNumComponents()
    ...
    if( sMaskImage != null ) {
      ...
      BufferedImage outImage = new BufferedImage( w,
                                                  h,
                                                  BufferedImage.TYPE_INT_ARGB );
      ...
      for( int i = 0; i < h; i++ ) {
        ...
        for( int j = 0; j < w; j++ ) {
          ...
          maskArray[ j ] =
            ((maskArray[ j ] & 0xff) << 24) | (srcArray[ j ] & ~ac);
        }
        // write pixel row
  ...
  private BufferedImage scaleSMaskImage( PDFImage sMaskImage )
    ...
    BufferedImage after = new BufferedImage( w,
                                             h,
                                             BufferedImage.TYPE_INT_ARGB );
  ...
  private BufferedImage convertGreyscaleToArgb( byte[] data,
                                                BufferedImage bi ) {
    ...
    return new BufferedImage( ccm,
                              Raster.createPackedRaster( new DataBufferInt(
                                                           convertedPixels,
                                                           convertedPixels.length ),
                                                         getWidth(),
                                                         getHeight(),
                                                         getWidth(),
                                                         ((PackedColorModel) ccm)
                                                           .getMasks(),
                                                         null ),
                              false, null );
  }
  ...
  private static int[] createGreyToArgbMap( int numBits ) {
    ...
    final WritableRaster inRaster =
      Raster.createPackedRaster( new DataBufferByte(
                                   greyVals,
                                   greyVals.length ),
                                 greyVals.length,
                                 1,
    ...
    final BufferedImage greyImage =
      new BufferedImage( new PdfComponentColorModel(
        greyCs,
        new int[]{numBits} ),
                         inRaster, false, null );

    final ColorModel ccm = ColorModel.getRGBdefault();
    final WritableRaster outRaster =
      Raster.createPackedRaster( new DataBufferInt(
                                   argbVals,
                                   argbVals.length ),
                                 argbVals.length,
                                 ...
                                 ((PackedColorModel) ccm)
                                   .getMasks(),
                                 null );
    final BufferedImage srgbImage = new BufferedImage( ccm,
                                                       outRaster,
                                                       false,
                                                       null );
  ...
  /**
   * Creates a new image of type {@link BufferedImage#TYPE_BYTE_GRAY} which
   * represents the
   * given raster
   *
  ...
  private BufferedImage biColorToGrayscale( final WritableRaster raster,
    ...
    byte[] buffer = ((DataBufferByte) converted.getRaster()
  ...
  private float[] normalize( byte[] pixels, float[] normComponents,
    ...
    for( int i = 0; i < pixels.length; i++ ) {
      ...
      normComponents[ normOffset + i ] = FunctionType0.interpolate( val,
                                                                    0,
                                                                    pow,
                                                                    ymin,
                                                                    ymax );
  ...
  class DecodeComponentColorModel extends ComponentColorModel {
    ...
    public float[] getNormalizedComponents( Object pixel,
                                            ...
                                            int normOffset ) {
      if( getDecode() == null ) {
        return super.getNormalizedComponents( pixel,
                                              normComponents,
                                              normOffset );
      }
  ...
  private ColorModel createColorModel() {
    ...
    if( cs instanceof IndexedColor ) {
      ...
      return decode != null ? new DecodeComponentColorModel( cs.getColorSpace(),
                                                             bits )
  ...
  private class JpegDecoder {
     ...
     * @throws IOException if the image couldn't be decoded due to a lack of
     *                     support
     *                     or some IO problem
     */
    ...
    private BufferedImage readImage( ImageReader jpegReader,
      ...
      if( ycckcmykDecodeMode ) {
        ...
        if( imageMeta != null ) {
          final Node standardMeta =
            imageMeta.getAsTree( IIOMetadataFormatImpl.standardMetadataFormatName );
          if( standardMeta != null ) {
            final Node chroma = getChild( standardMeta, "Chroma" );
            if( chroma != null ) {
              final Node csType = getChild( chroma, "ColorSpaceType" );
              if( csType != null ) {
                final Attr csTypeNameNode = (Attr) csType.getAttributes()
                                                         .getNamedItem( "name"
                                                         );
                if( csTypeNameNode != null ) {
                  final String typeName = csTypeNameNode.getValue();
                  final boolean YCCK;
                  if( (YCCK = "YCCK".equals( typeName )) || "CMYK".equals(
                    typeName ) ) {
        ...
        if( param != null && param.getDestination() != null ) {
          ...
          // desired color model
          //return new BufferedImage(cm, jpegReader.read(0, param).getRaster
          // (), true, null);
  ...
  static class PdfComponentColorModel extends ComponentColorModel {
    ...
    public SampleModel createCompatibleSampleModel( int width, int height ) {
      ...
      if( bitsPerComponent >= 8 ) {
        ...
        return new PixelInterleavedSampleModel( getTransferType(),
                                                width,
                                                height,
                                                numComponents,
                                                width * numComponents,
        ...
        switch( getPixelSize() ) {
          ...
          default:
            ...
            return new PdfSubByteSampleModel( width,
                                              height,
                                              getNumComponents(),
                                              bitsPerComponent );
        }
