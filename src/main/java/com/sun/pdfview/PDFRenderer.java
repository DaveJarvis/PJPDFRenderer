...
public class PDFRenderer extends BaseWatchable implements Runnable {
  ...
  private void setupRendering( Graphics2D g ) {
    ...
    mState = new GraphicsState();
    mState.cliprgn = null;
    mState.stroke = new BasicStroke();
    mState.strokePaint = PDFPaint.getColorPaint( Color.black );
    mState.fillPaint = mState.strokePaint;
    mState.fillAlpha = AlphaComposite.getInstance( AlphaComposite.SRC );
    mState.strokeAlpha = AlphaComposite.getInstance( AlphaComposite.SRC );
    mState.xform = g.getTransform();
   ...
   */
  public void push() {
    final var state = mState;

    if( state != null ) {
      state.cliprgn = g.getClip();
      stack.push( state );
      mState = (GraphicsState) state.clone();
    }
  }
  ...
  public void pop() {
    ...
    setTransform( mState.xform );
    setClip( mState.cliprgn );
  }
  ...
  public Rectangle2D stroke( GeneralPath s ) {
    g.setComposite( mState.strokeAlpha );
    s = new GeneralPath( autoAdjustStrokeWidth( g,
                                                mState.stroke ).createStrokedShape(
      s ) );
    return mState.strokePaint.fill( this, g, s );
  }
  ...
  public void draw( GeneralPath p, BasicStroke bs ) {
    g.setComposite( mState.fillAlpha );
    g.setPaint( mState.fillPaint.getPaint() );
  ...
  public Rectangle2D fill( GeneralPath s ) {
    g.setComposite( mState.fillAlpha );
    return mState.fillPaint.fill( this, g, s );
  }
   ...
   *              and ary must both be valid, or phase must be NOPHASE while
   *              ary is null.
   */
  public void setStrokeParts( float w, int cap, int join, float limit,
                              float[] ary, float phase ) {
    if( w == NOWIDTH ) {
      w = mState.stroke.getLineWidth();
    }
    if( cap == NOCAP ) {
      cap = mState.stroke.getEndCap();
    }
    if( join == NOJOIN ) {
      join = mState.stroke.getLineJoin();
    }
    if( limit == NOLIMIT ) {
      limit = mState.stroke.getMiterLimit();
    }
    if( phase == NOPHASE ) {
      ary = mState.stroke.getDashArray();
      phase = mState.stroke.getDashPhase();
    }
    if( ary != null && ary.length == 0 ) {
      ary = null;
    }
    if( phase == NOPHASE ) {
      mState.stroke = new BasicStroke( w, cap, join, limit );
    }
    else {
      mState.stroke = new BasicStroke( w, cap, join, limit, ary, phase );
    }
  }
  ...
  public void setStrokeAlpha( float alpha ) {
    mState.strokeAlpha = AlphaComposite.getInstance( AlphaComposite.SRC_OVER,
                                                     alpha );
  }
  ...
  public void setFillAlpha( float alpha ) {
    mState.fillAlpha = AlphaComposite.getInstance( AlphaComposite.SRC_OVER,
                                                   alpha );
  }
  ...
  private BufferedImage getMaskedImage( BufferedImage bi ) {
    ...
    Color col = (Color) mState.fillPaint.getPaint();
