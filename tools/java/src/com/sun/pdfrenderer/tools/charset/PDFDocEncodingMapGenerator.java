...
public class PDFDocEncodingMapGenerator {
  ...
  public static void main( String[] args ) throws IOException {
    ...
    final BufferedReader r = new BufferedReader( new InputStreamReader(
      PDFDocEncodingMapGenerator.class.getResourceAsStream(
        "PDFDocEncodingMap.txt" ) ) );
    ...
    for( int i = 0; i < 256; ++i ) {
      ...
      if( nameLine != null ) {
        if( !undefinedCh ) {
          ...
          if( mappedCh == null || mappedCh != unicodeCh ) {
            if( mappedCh != null ) {
              problemMappings.add( i );
            }
            else {
      ...
      if( unicodeCh >= Character.MIN_SUPPLEMENTARY_CODE_POINT ) {
        // TODO: needs to use a surrogate, so can't be expressed as a single
    ...
    final BufferedReader r = new BufferedReader(
      new InputStreamReader(
        PDFDocEncodingMapGenerator.class.
                                          getResourceAsStream( "UnicodeData" +
                                                                 ".txt" ) ) );
    String line;
